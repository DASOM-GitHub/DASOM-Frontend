...
const Apply = () => {
	...
	const isFormComplete = inputName && studentId && tel && motivation && Grade && Department;
		...
		<div className="apply-main">
			...
			<form className="apply-ques-box" onSubmit={handleSubmit}>
				<input
					...
					value={inputName}
					placeholder="이름"
					onChange={handleChange(setInputName)}></input>
				<input
					className="apply-input-text"
					placeholder="학번"
					value={studentId}
					onChange={handleChange(setStudentId)}></input>
				<div className="apply-grade-box">
					{['1학년', '2학년', '3학년'].map((grade) => (
						<label key={grade} className={`apply-grade ${Grade === grade ? 'selected' : ''}`}>
							<input
								type="radio"
								name="grade"
								value={grade}
				...
				<div className="apply-department-box">
					{['컴퓨터소프트웨어공학과', '인공지능소프트웨어공학과', '컴퓨터정보공학과'].map((department) => (
						<label key={department} className={`apply-grade ${Department === department ? 'selected' : ''}`}>
							<input
								type="radio"
								value={department}
				...
				<input className="apply-input-text" placeholder="연락처" value={tel} onChange={handleChange(setTel)}></input>
				<textarea
					ref={textRef}
					...
					rows={0}></textarea>
				<input type="submit" className="apply-submit" value="지원하기" disabled={!isFormComplete}></input>
			</form>
		</div>
	);
};
